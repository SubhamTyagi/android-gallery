...
/**
 * The main Crypto API for encryption and decryption of byte arrays and files.
 */
public class Crypto {
    ...
    /**
     * Initializes a new {@code Crypto} object for encryption and decryption. See
     * {@link org.horaapps.leafpic.encryption.Crypto} for an explanation of options.
     *
     * @param context an {@link org.horaapps.leafpic.encryption.CryptoContext}
     */
    @SuppressWarnings("WeakerAccess")
    public Crypto(CryptoContext context) {
        if (context == null ||
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public static byte[] generateKey(CryptoContext.Algorithm algorithm, CryptoContext.KeyLength keyLength)
        ...
        if (algorithm == CryptoContext.Algorithm.DES) {
            actualKeyLength -= 8;
        }
    ...
    /**
     * Gets a {@link Mac} instance
     *
     * @param macAlgorithm the {@link org.horaapps.leafpic.encryption.CryptoContext.MacAlgorithm}
     * @param password     a password
     * @return an initialized {@link Mac}
     * @throws GeneralSecurityException if MAC initialization fails
     */
    private Mac getMac(CryptoContext.MacAlgorithm macAlgorithm, char[] password) throws GeneralSecurityException {
    ...
    public synchronized byte[] encrypt(byte[] input, char[] password) throws GeneralSecurityException, IOException {
        ...
        if (context.getMacAlgorithm() != CryptoContext.MacAlgorithm.NONE) {
     ...
     * Encrypts the input file using the supplied password
     *
     * @param inputString the input plain string
     * @param password    the password
     * @return the encrypted string
     * @throws IOException           if there's a failure to read/write from/to the input/output stream
     * @throws GeneralSecurityException  if initialization or decryption fails
     */
    public String encrypt(String inputString, char[] password) throws IOException, GeneralSecurityException {
        byte[] bytes = inputString.getBytes(StandardCharsets.UTF_8);
        return Base64.encodeToString(encrypt(bytes, password), Base64.DEFAULT);
    }
     ...
     * @throws GeneralSecurityException  if initialization or decryption fails
     */
    public String decrypt(String inputString, char[] password) throws IOException, GeneralSecurityException {
        byte[] bytes = Base64.decode(inputString, Base64.DEFAULT);
        return new String(decrypt(bytes, password), StandardCharsets.UTF_8);
    }


    /**
     * Decrypt Files from folder.
     *
     ...
     * @throws GeneralSecurityException  if initialization or decryption fails
     */
    public void decryptFromFolder(File inputFolderName, File outputFolderName, char[] password) throws GeneralSecurityException, IOException {
        File[] files = inputFolderName.listFiles();
        if (files != null) {
            for (File fileEntry : files) {
                if (!fileEntry.isDirectory()) {
                    String fileOutputName = outputFolderName.getAbsolutePath() + "/" + fileEntry.getName().replaceFirst("[.][^.]+$", "");
                    decrypt(fileEntry, new File(fileOutputName), password);
                }
            }
        }
    ...
    /**
     * Encrypt the files under folder.
     *
     * @param inputFolderName  the input folder file which
     * @param outputFolderName the output folder name
     * @param password         the password
     ...
     * @throws GeneralSecurityException  if initialization or decryption fails
     */
    public void encryptFromFolder(File inputFolderName, File outputFolderName, char[] password, String postfix) throws GeneralSecurityException, IOException {
        File[] files = inputFolderName.listFiles();
        if (files != null) {
            for (File fileEntry : files) {
                if (!fileEntry.isDirectory()) {
                    String fileOutputName = outputFolderName.getAbsolutePath() + "/" + fileEntry.getName() + postfix;
                    encrypt(fileEntry, new File(fileOutputName), password);
                }
            }
        }
    }
    ...
    private AlgorithmParameterSpec getAlgorithmParameterSpec(CryptoContext.Mode mode, byte[] initializationVector) {
        switch (mode) {
            case CBC:
            case CTR:
